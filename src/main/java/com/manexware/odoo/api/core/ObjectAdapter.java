package com.manexware.odoo.api.core;
...
import com.manexware.odoo.api.core.Field.FieldType;
import com.manexware.odoo.api.core.FilterCollection.FilterOperator;
import com.manexware.odoo.api.core.helpers.FilterHelper;
...
public class ObjectAdapter {
	 ...
	 * 
	 * @param odooSession
	 *            OdooSession object that will be used to make the calls
	 * @param objectName
	 *            Object name that this adapter will work for.
	 * @throws XmlRpcException
	 * @throws OdooApiException
	 */
	public ObjectAdapter(OdooSession odooSession, String objectName) {
		this.commands = odooSession.getOpenERPCommand();
		this.objectName = objectName;
		this.server_version = odooSession.getServerVersion();
	...
	private synchronized static void objectExists(OdooCommand commands,
			String objectName) {
		...
		if (objectNameCache.indexOf(objectName) < 0) {
			...
			try {
				Object[] ids = commands.searchObject("ir.model",
						new Object[] {});
				Object[] result = commands.readObject("ir.model", ids,
						new String[] { "model" });
				for (Object row : result) {
					objectNameCache.add(((HashMap<String, Object>) row).get(
							"model").toString());
				}
			} catch (XmlRpcException e) {
				throw new OdooApiException(
						"Could not validate model name: ", e);
			}
		}

		if (objectNameCache.indexOf(objectName) < 0)
			throw new OdooApiException("Could not find model with name '"
					+ objectName + "'");
	}
	 ...
	 * @throws OdooApiException
	 */
	public Row getNewRow(FieldCollection fields) throws XmlRpcException,
			OdooApiException {
	 ...
	 * @throws OdooApiException
	 */
	public Row getNewRow(String[] fields) {
	 ...
	 * @throws OdooApiException
	 */
	public RowCollection readObject(Object[] ids, String[] fields)
		 ...
		 * this section of code ArrayList<Integer> idList = new
		 * ArrayList<Integer>(); for (Object id : ids){
		 * idList.add(Integer.parseInt(id.toString())); } Object[] sortedResults
		 * = new Object[ids.length]; for (Object result : results){
		 * 
		 * @SuppressWarnings("unchecked") int id =
		 *                                Integer.parseInt(((HashMap<String,
		 *                                Object>)result).get("id").toString());
		 *                                sortedResults[idList.indexOf(id)] =
		 *                                result; }
		 ****/
	...
	public FieldCollection getFields(String[] filterFields) {
		...
		HashMap<String, Object> fields = commands.getFields(objectName,
				filterFields);

		for (String fieldName : fields.keySet()) {
			HashMap<String, Object> fieldDetails = (HashMap<String, Object>) fields
					.get(fieldName);
	 ...
	 * @throws OdooApiException
	 */
	public Object[] validateFilters(final FilterCollection filters)
			throws OdooApiException {
		...
		for (int i = 0; i < filters.getFilters().length; i++) {
			...
			if (filter == null)
				throw new OdooApiException(
						"The first filter parameter is mandatory");
			...
			if (!(filter instanceof Object[])
					&& ((Object[]) filter).length != 3)
				throw new OdooApiException(
						"Filters aren't in the correct format.  Please read the OpenERP help.");
			...
			if (fld != null && fld.getFunc_method() == true)
				throw new OdooApiException(
						"Can not search on function field " + fieldName);

			// Fix the value type if required for the OpenERP server
			if (!fieldName.equals("id") && fld == null)
				throw new OdooApiException("Unknow filter field "
						+ fieldName);
				...
				comparison = "!=";
				value = false;
			} else if (fld != null && fld.getType() == FieldType.BOOLEAN
					&& !(value instanceof Boolean)) {
				if (value instanceof String) {
					char firstchar = value.toString().toLowerCase().charAt(0);
					if (firstchar == '1' || firstchar == 'y'
							|| firstchar == 't')
						value = true;
					else if (firstchar == '0' || firstchar == 'n'
							|| firstchar == 'f')
						value = false;
					else
						throw new OdooApiException("Unknown boolean "
								+ value.toString());
				}
			} else if (fld != null && fld.getType() == FieldType.FLOAT
				...
				if (!(value instanceof Integer)) {
					if (fieldName.equals("id")
							|| (fld != null
									&& fld.getType() == FieldType.INTEGER && !(value instanceof Integer))
							|| (fld != null
									&& fld.getType() == FieldType.MANY2ONE && !(value instanceof Integer))) {
				...
				if (value instanceof String) {
					...
					for (int entrIdx = 0; entrIdx < entries.length; entrIdx++) {
						String entry = FilterHelper
								.csvDecodeString(entries[entrIdx]);
						...
						if (fld != null
								&& (fld.getType() == FieldType.INTEGER
										...
										|| fld.getType() == FieldType.MANY2MANY || fld
										.getType() == FieldType.MANY2ONE)
								|| fieldName.equals("id")) {
	...
	private Object[] fixImportData(Row inputRow) throws OdooApiException,
		...
		for (int i = 0; i < inputRow.getFields().size(); i++) {
			...
			case SELECTION:
				...
				if (!validValue)
					throw new OdooApiException(
							"Could not find a valid value for section field "
				...
				if (value instanceof String) {
					for (String singleID : value.toString().split(","))
						if (idToName.containsKey(singleID))
							...
							throw new OdooApiException("Could not find "
									+ fld.getRelation() + " with ID "
									+ singleID);
				} else {
					...
					for (Object singleID : (Object[]) value)
						if (idToName.containsKey(singleID.toString()))
							newValue = newValue + ","
									+ idToName.get(singleID.toString());
						else
							throw new OdooApiException("Could not find "
									+ fld.getRelation() + " with ID "
									+ singleID.toString());
	...
	public boolean importData(RowCollection rows) throws OdooApiException,
		...
		if (this.server_version.getMajor() >= 7
				&& this.server_version.getMinor() == 0) {
		...
		String[] targetFieldList = getFieldListForImport(rows.get(0)
				.getFields());
		...
		if (this.server_version.getMajor() >= 7) {
			...
			if (this.server_version.getMinor() == 0 && rows.size() > 0
					&& rows.get(0).getID() == 0) {
			...
			HashMap<String, Object> results = commands.Load(objectName,
					targetFieldList, importRows);
			...
			if (results.get("ids") instanceof Boolean) {
				...
				for (Object mes : messages) {
					...
					errorString.append("Row: "
							+ messageHash.get("record").toString() + " field: "
							+ messageHash.get("field").toString() + " ERROR: "
							+ messageHash.get("message").toString() + "\n");
				}
				throw new OdooApiException(errorString.toString());
			}
			...
			Object[] result = commands.importData(objectName, targetFieldList,
					importRows);
	 ...
	 * 
	 * @param filter
	 *            A filter collection that contains a list of filters to be
	 *            applied
	 * @return The number of record count.
	 * @throws XmlRpcException
	 * @throws OdooApiException
	 */
	public int getObjectCount(FilterCollection filter) throws XmlRpcException,
			OdooApiException {
		Object[] preparedFilters = validateFilters(filter);
		return Integer.parseInt(commands.searchObject(objectName,
				preparedFilters, -1, -1, null, true).toString());
	}
	 ...
	 * @throws OdooApiException
	 */
	public RowCollection searchAndReadObject(FilterCollection filter,
			String[] fields) throws XmlRpcException, OdooApiException {
	 ...
	 *            List of fields to return data for
	 * @param offset
	 *            Number of records to skip. -1 for no offset.
	 * @param limit
	 ...
	 * @throws OdooApiException
	 */
	public RowCollection searchAndReadObject(final FilterCollection filter,
			final String[] fields, int offset, int limit, String order)
			throws XmlRpcException, OdooApiException {

		String[] fieldArray = (fields == null ? new String[] {} : fields);
		Object[] preparedFilters = validateFilters(filter);
		Object[] idList = (Object[]) commands.searchObject(objectName,
				preparedFilters, offset, limit, order, false);
	 ...
	 * @throws XmlRpcException
	 */
//	public Boolean[] writeObject(final RowCollection rows,
//			final boolean changesOnly) throws OdooApiException,
//			XmlRpcException {
...
//	}
	 ...
	 * @throws XmlRpcException
	 */
	public boolean writeObject(final Row row, boolean changesOnly)
		...
		if (idObj == null || Integer.parseInt(idObj.toString()) <= 0)
			throw new OdooApiException(
					"Please set the id field with the database ID of the object");

		int id = Integer.parseInt(idObj.toString());

		if (changesOnly) {
			for (Field fld : row.getChangedFields())
				valueList.put(fld.getName(),
						formatValueForWrite(fld, row.get(fld)));
		} else
	 ...
	 * @throws XmlRpcException
	 */
	public void createObject(final Row row) {

		HashMap<String, Object> valueList = new HashMap<String, Object>();
		for (Field fld : row.getFields())
			valueList
					.put(fld.getName(), formatValueForWrite(fld, row.get(fld)));

		if (valueList.size() == 0)
			throw new OdooApiException(
					"Row doesn't have any fields to update");
	 ...
	 */
	@SuppressWarnings("unchecked")
	public FieldCollection callFieldsFunction(String functionName,
			Object[] parameters) throws XmlRpcException, OdooApiException {
		Object[] results = commands.callObjectFunction(objectName,
		...
		if (results.length > 0) {
			...
			for (String field : rowMap.keySet()) {
				...
				if (rowMap.get(field) instanceof HashMap<?, ?>) {
					try {
						fldDetails = (HashMap<String, Object>) rowMap
								.get(field);
					} catch (Exception e) {
						fldDetails = null;
					}
	 ...
	 * @throws OdooApiException
	 */
	public RowCollection callFunction(String functionName, Object[] parameters,
			FieldCollection fieldCol) throws XmlRpcException,
			OdooApiException {
		Object[] results = commands.callObjectFunction(objectName,
				functionName, parameters);
